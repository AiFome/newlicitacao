...
// Página de monitoramento por palavras-chave
import { useState } from 'react'
import useSWR from 'swr'
import { licitacoesApi, alertasApi } from '@/lib/api'
...
import {
  Search, Plus, X, ExternalLink, Bell, TrendingUp,
...
export default function MonitorarPage() {
  ...
  function adicionarPalavra() {
    ...
    if (!p || palavras.includes(p)) return
  ...
  async function buscar() {
    if (palavras.length === 0) { toast.error('Adicione pelo menos uma palavra-chave'); return }
    setBuscando(true)
    try {
      const q = palavras.join(' ')
      const res = await licitacoesApi.buscar({ q, situacao: 'ABERTA', limit: 30, sort: 'recentes' })
  ...
  async function criarAlerta() {
    ...
    try {
      ...
      toast.success('Alerta criado! Você será notificado quando sair novos editais.')
    } catch (err: any) {
  ...
  return (
    <div className="p-6 max-w-5xl">
      <div className="mb-8">
        ...
        <p className="text-sm text-muted">Adicione palavras e encontre editais que correspondem ao seu interesse</p>
      </div>

      {/* Input de palavras */}
      <div className="card p-5 mb-6">
        <label className="block text-xs font-600 text-muted uppercase tracking-wide mb-3">Palavras-chave</label>
        ...
        <div className="flex gap-2 mb-4">
          ...
          />
          <button onClick={adicionarPalavra} className="btn-secondary gap-2 px-4 shrink-0">
        ...
        {palavras.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {palavras.map((p) => (
              <span key={p} className="flex items-center gap-1.5 rounded-full px-3 py-1 text-sm font-500"
                style={{ background: 'var(--brand-50)', color: 'var(--brand-700)', border: '1px solid var(--brand-200)' }}>
                <Tag size={11} />
                {p}
                <button onClick={() => removerPalavra(p)} className="opacity-50 hover:opacity-100 transition-opacity">
                  <X size={13} />
                </button>
              </span>
            ))}
        ...
        <div className="flex gap-3">
          <button
            ...
            {buscando ? <><Loader2 size={14} className="animate-spin" />Buscando...</> : <><Search size={14} />Buscar editais</>}
          </button>
          ...
          {palavras.length > 0 && (
            <button
              ...
              {criandoAlerta ? <><Loader2 size={14} className="animate-spin" />Criando...</> : <><Bell size={14} />Criar alerta automático</>}
            </button>
          )}
        </div>
        ...
        {palavras.length > 0 && (
          <p className="text-xs text-muted mt-3">
            ...
            O alerta automático vai te notificar por e-mail quando sair novos editais com essas palavras
          </p>
        )}
      </div>
        ...
        <div className="animate-fade-up">
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm font-600 text-text">
              {resultados.length === 0
                ? 'Nenhum edital encontrado com essas palavras'
                : `${resultados.length} editais encontrados`}
            </p>
          </div>
          ...
          {resultados.length === 0 ? (
            <div className="card p-12 flex flex-col items-center gap-3 text-center">
              ...
              <button onClick={criarAlerta} className="btn-primary gap-2 px-5 mt-2" style={{ background: 'var(--brand-600)' }}>
            ...
            <div className="space-y-3">
              {resultados.map((l) => (
                <Link key={l.id} href={`/licitacoes/${l.id}`}
                  className="card p-5 flex items-start gap-4 hover:border-brand-200 hover:shadow-hover transition-all duration-200 group">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start gap-2 mb-2 flex-wrap">
                      ...
                      <span className="text-[11px] text-muted">{PORTAL_LABEL[l.portalOrigem] ?? l.portalOrigem}</span>
                    ...
                    <p className="text-xs text-muted">{l.orgao?.razaoSocial}</p>
                    <div className="flex items-center gap-4 mt-2 text-xs text-muted">
                      {l.valorEstimado && <span className="font-500 text-text">{formatMoedaCompacta(l.valorEstimado)}</span>}
      ...
      {alertas && alertas.length > 0 && (
        <div className="mt-10">
          <h2 className="text-sm font-600 text-text mb-4 flex items-center gap-2">
            ...
            Seus alertas ativos ({alertas.length})
          </h2>
          <div className="space-y-2">
            {alertas.slice(0, 5).map((a: any) => (
              <div key={a.id} className="card p-4 flex items-center gap-3">
                <div className={clsx('h-2 w-2 rounded-full shrink-0', a.ativo ? 'bg-green-500' : 'bg-surface-border')} />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-500 text-text">{a.nome}</p>
                  <p className="text-xs text-muted truncate">{a.palavrasChave?.join(', ')}</p>
                </div>
                <Link href="/alertas" className="text-xs text-brand-600 hover:underline">Gerenciar</Link>
              </div>
            ))}
          </div>
        </div>
